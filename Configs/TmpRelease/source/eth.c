...
/*Direccion MAC Multicast*/
mac_addr_t  RIP_MAC_MCAST_ADDR = {0x01,0x00,0x5e,0x00,0x00,0x09};
...
/* Minimo tamaño de frame ethernet */
#define ETH_FRAME_MIN_LENGTH 64
/* Minimo tamaño del pauload de ethernet */
#define ETH_MIN_PAYLOAD_SIZE ETH_FRAME_MIN_LENGTH - ETH_HEADER_SIZE
  ...
  mac_addr_t dst, uint16_t type, unsigned char * payload, int payload_len )
{
  ...
  /*********PADDING**********/
  if (payload_len < ETH_MIN_PAYLOAD_SIZE)
  {
	unsigned char aux[ETH_MIN_PAYLOAD_SIZE];
	memset(aux, 0, ETH_MIN_PAYLOAD_SIZE);
	memcpy(aux, payload, payload_len);
	payload_len = ETH_MIN_PAYLOAD_SIZE;
	payload = aux;
  }
  /**************************/
...
int check(mac_addr_t mac_addr,struct eth_iface *iface)
{
  
	int is_my_group_mac = 0;

	is_my_group_mac = ((memcmp(mac_addr,iface->mac_address, MAC_ADDR_SIZE) == 0)
			    ||(memcmp(mac_addr,RIP_MAC_MCAST_ADDR, MAC_ADDR_SIZE) == 0)
			    ||(memcmp(mac_addr,MAC_BCAST_ADDR, MAC_ADDR_SIZE) == 0));
  
  return is_my_group_mac;
}
  ...
  mac_addr_t src, uint16_t type, unsigned char buffer[], long int timeout )
{
  ...
  do {
    ...
    ////////////////////*Aqui hay que a�adir multicast\\\\\\\\\\\\\\\\*/
    eth_frame_ptr = (struct eth_frame *) eth_buffer;
    is_my_mac = check(eth_frame_ptr->dest_addr, iface); 
//    is_my_mac = (memcmp(eth_frame_ptr->dest_addr, iface->mac_address, MAC_ADDR_SIZE) == 0); // ! check_mac(eth_frame_ptr->dest_addr) 
