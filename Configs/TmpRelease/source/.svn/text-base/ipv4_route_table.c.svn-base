...
int ipv4_route_lookup ( ipv4_route_t * route, ipv4_addr_t addr )
{
	...
	uint32_t netmask = ipv4_addr_bin(route->subnet_mask);
	uint32_t prefix = ipv4_addr_bin(route->subnet_addr);
...
void ipv4_route_print ( ipv4_route_t * route )
{
  if (route != NULL) {
    ...
    char gw_str[IPv4_ADDR_STR_LENGTH];
  ...
  while ((! feof(routes_file)) && (err==0)) {
    ...
    if (err != 4) {
      ...
      ipv4_route_t * new_route = 
        ipv4_route_create(subnet, mask, iface_name, gateway);
