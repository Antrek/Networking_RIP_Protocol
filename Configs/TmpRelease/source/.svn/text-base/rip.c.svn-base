			 ...
			 unsigned char *buffer, unsigned char *opcode, long int timeout)
{
	...
	else
	{
		...
		/**** DEBERIA COMPROBAR TAMBIEN QUE NO VENGA DE NUESTRA IP (MULTICAST)****/
			 ...
			 int len, int op_code) 
{
	...
	}else if(op_code == RIP_RESPONSE)
	{
		...
		// Eliminamos, si existe, la ruta a la subred a la que va el RESPONSE
		int i, found = 0;
		for (i = 0; i < len/sizeof(rip_route_t); i++)
		{
			// Creamos la ruta ipv4_route de la tabla para analizarla con lookup
			ipv4_route_t ip_route;
			rip_route_t *aux_entry = &(((struct rip_datagram *)rip_datagram)->rip_entry[i]);
			memcpy(ip_route.subnet_addr, &(aux_entry->ip_addr), IPv4_ADDR_SIZE);
			memcpy(ip_route.subnet_mask, &(aux_entry->mask), IPv4_ADDR_SIZE);
			
			if (ipv4_route_lookup(&ip_route, dst) >= 0)
			{
				found = 1;
				break;
			}	
		...
		if (found)
		{
			rip_route_t *aux_routes = ((struct rip_datagram *)rip_datagram)->rip_entry;

			memcpy(&(aux_routes[i]), &(aux_routes[len/sizeof(rip_route_t) - 1]), sizeof(rip_route_t));
			len -= sizeof(rip_route_t);
			total_len -= sizeof(rip_route_t);

			memcpy(((struct rip_datagram *)rip_datagram)->rip_entry, aux_routes, len);

			//&(((struct rip_datagram *)rip_datagram)->rip_entry[i] = NULL;
		}
		...
		{
			/******** POISONED REVERSE ***********/
			rip_route_t *aux = &(((struct rip_datagram *)rip_datagram)->rip_entry[i]);
			if (memcmp(aux->next_hop, dst, IPv4_ADDR_SIZE) == 0)
			{	aux->metric = 16;
				aux->metric = htonl(aux->metric);
			}
			/*************************************/
				
			memset(aux->next_hop, 0, IPv4_ADDR_SIZE);
			//rip_route_print (&(((struct rip_datagram *)rip_datagram)->rip_entry[i]) );
		}
