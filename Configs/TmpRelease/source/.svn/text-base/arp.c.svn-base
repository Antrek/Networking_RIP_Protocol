...
{
	/************ ARP Cache ************/
	puts("#DEBUG# Arp resolve");	
	puts("#Tabla de cache ARP#");
	arp_table_print();
	
	arp_entry_t *entry = arp_table_search(dest);
	if (entry != NULL && (int)(timerms_elapsed(&(entry->timer))/1000) < ARP_MAX_TIMEOUT)
	{
	...
	// Actualiza la cache ARP
	arp_table_update(req.ip_dst, mac, eth_getname(iface));
