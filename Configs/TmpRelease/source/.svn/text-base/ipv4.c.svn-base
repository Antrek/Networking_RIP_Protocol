...
/* Declaracion global de la ip Multicast para routres */ 
ipv4_addr_t RIP_MCAST_ADDR = { 224, 0, 0, 9 };
...
*
* DESCRIPCIÓN:
*   Esta función local realiza la conversión de una dirección en binario a String
*
* PARÁMETROS:
*   'ipv4_addr': Cadena de texto donde escribir
*   uint32_t ipv4_bin_addr : La direccion ip en binario.
*
* VALOR DEVUELTO:
*   Devuelve la representación binaria de la dirección pasada por parámetros.
*/
...
{	
	...
	// Al abrir IP crea una nueva tabla de cache ARP
	arp_new_table();
	/*******************************/
				...
				int payload_len )
{
	...
	int status;
    if(memcmp(ipv4_dst,RIP_MCAST_ADDR,IPv4_ADDR_SIZE)==0) 
    {
        status = 1;
        memcpy(mac_dst,RIP_MAC_MCAST_ADDR,MAC_ADDR_SIZE);
    }
    else
    {
        status = arp_resolve(ipv4_handler.iface,ipv4_handler.ipv4_src, arp_addr,
						 mac_dst);
    }
			   ...
			   long int timeout )
{
	...
	do
	{
		...
		/*
		int checksum_ok = verify_checksum((unsigned char *) ip_buffer, IPv4_HEAD_SIZE);
		if( checksum_ok == 0)
		{
			fprintf(stderr, "Datagrama corrupto, fallo al comprobar el checksum\n");
			return -1;
		}
        */
		...
		if((memcmp(ip_dgm->ipv4_src, ipv4_handler.ipv4_src, IPv4_ADDR_SIZE) == 0))
			recv = 0;
		else if (ip_dgm->protocol == protocol)
		{
			if (memcmp(ip_dgm->ipv4_dst, RIP_MCAST_ADDR, IPv4_ADDR_SIZE) == 0 ||
				memcmp(ip_dgm->ipv4_dst, ipv4_handler.ipv4_src, IPv4_ADDR_SIZE) == 0)
				recv = 1;
		}
